...
class Connect:
    ...
    def body(self, pagina):
        ...
        #menu = primanota[1]["descrizione"]
        ###return body
        body = [dict(id=row[0], author_id=row[1],slug=row[2], titolo=row[3],testo=row[5], img2=row[12],testo2=row[10]) for row in rows]
        return body
    ...
    def slider(self, luogo):
        ...
        rows = cursor.fetchall()
        slider = [dict(id=row[0], codice=row[1],img=row[2], titolo=row[3],caption=row[4], link=row[5]) for row in rows]
