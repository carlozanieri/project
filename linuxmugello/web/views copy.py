...
def newss(request):
        ...
        context = {'pagina':Connect.body("", "sanpiero"), 'manifestazione':"web", 'news':Connect.news(""), 'urlx':"html"}
        return render(request, 'web/news.html',context)
...
def news_one(request):
        ...
        return render(request, 'web/news_one.html',context)
            
def slide(request):
        luogo="sanpiero"
        context = {'luogo' : luogo, 'slider':Connect.slider("", "index"),'menu': Connect.menu("")}
        return render(request, 'web/nivo.html',context)      